...
def process_single_image_fallback(args):
    """単一画像の処理（フォールバック版）"""
    ...
    try:
        ...
        if len(faces) > 0:
            ...
            # 顔検出失敗：画像比率ベースのフォールバック
            print(f"顔検出失敗、フォールバック処理: {image_path}")
            ...
            # 横棒（眉毛～目領域）
            eyebrow_top = int(center_y - height * 0.25)
            eye_bottom = int(center_y - height * 0.05)
            horizontal_left = int(width * 0.15)
            horizontal_right = int(width * 0.85)
            
            # 縦棒（鼻～口領域）
            vertical_top = int(center_y - height * 0.15)
            vertical_bottom = int(center_y + height * 0.15)
            vertical_left = int(center_x - width * 0.12)
            vertical_right = int(center_x + width * 0.12)
...
def main():
    parser = argparse.ArgumentParser(description='フォールバック並列T字マスク処理')
    parser.add_argument('input', help='入力ディレクトリのパス')
    parser.add_argument('-o', '--output', help='出力先のパス', default='organized_masked_fallback')
    ...
    print("フォールバックモード: 顔検出失敗時は画像比率ベースでマスク適用")
    ...
    for image_path in unprocessed_files:
        ...
        process_args.append((image_path, output_file, predictor_path))
    ...
    with ProcessPoolExecutor(max_workers=args.jobs) as executor:
        futures = {executor.submit(process_single_image_fallback, arg): arg for arg in process_args}
        
        for future in tqdm(as_completed(futures), total=len(futures), desc="フォールバック処理中"):
            try:
