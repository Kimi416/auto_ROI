...
class BCCFaceMaskAnnotator:
    def __init__(self):
        ...
        try:
            ...
            print("⚠️ dlib顔検出器が利用できません（体幹部のみ処理）")
        ...
        # 顔面・体幹部を分類
        self.classify_face_trunk_files()
        
        # 進行状況を読み込み
        self.load_progress()
        
    def classify_face_trunk_files(self):
        """ファイルを顔面・体幹部に分類"""
        self.face_files = []
        self.trunk_files = []
        ...
        for file_info in self.bcc_files:
        ...
        print(f"📊 分類結果:")
        print(f"  顔面画像: {len(self.face_files)}枚")
        print(f"  体幹部画像: {len(self.trunk_files)}枚")
        
        # 全体のファイルリストを更新（顔面を先に処理）
        self.bcc_files = self.face_files + self.trunk_files
        
    def apply_t_mask(self, image):
        """T字マスクを適用"""
        ...
        for face in faces:
            ...
            # 目をマスク
            if len(left_eye) > 0:
                cv2.fillPoly(mask, [cv2.convexHull(left_eye)], 255)
            if len(right_eye) > 0:
                cv2.fillPoly(mask, [cv2.convexHull(right_eye)], 255)
            
            # 鼻をマスク
            if len(nose) > 0:
                cv2.fillPoly(mask, [cv2.convexHull(nose)], 255)
            
            # 口をマスク
            if len(mouth) > 0:
                cv2.fillPoly(mask, [cv2.convexHull(mouth)], 255)
    ...
    def show_help(self):
        """ヘルプを表示"""
        print("\n" + "="*60)
        print("🖱️  BCC 顔面マスク+アノテーション")
        print("="*60)
        print("🎭 顔面画像: 自動でT字マスク適用後にアノテーション")
        print("🫁 体幹部画像: そのままアノテーション")
        print("")
        print("左クリック       : 頂点を追加")
    ...
    def annotate_bcc_files(self):
        ...
        while file_index < len(self.bcc_files):
            ...
            # 顔面画像の場合はマスク適用
            if file_info.get('is_face', False):
                print(f"🎭 顔面画像: T字マスク適用中...")
                processed_image = self.apply_t_mask(self.current_original)
            else:
                print(f"🫁 体幹部画像: マスクなし")
                processed_image = self.current_original.copy()
            ...
            if width > 1200:
                ...
                self.current_image = cv2.resize(processed_image, (new_width, new_height))
            else:
            ...
            image_type = "顔面(マスク済み)" if file_info.get('is_face', False) else "体幹部"
            window_name = f"BCC Annotation [{file_index+1}/{len(self.bcc_files)}] {image_type} - {self.current_path.name}"
            cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
            cv2.resizeWindow(window_name, 1000, 700)
            cv2.setMouseCallback(window_name, self.mouse_callback)
            
            total_done = self.current_index + file_index
            print(f"\n📸 [完了: {total_done}, 残り: {len(self.bcc_files) - file_index}] {image_type} - {self.current_path.name}")
            print("BCC領域を左クリックで囲み、右クリックで完成...")
            ...
            while True:
                ...
                # 画像タイプ表示
                type_text = f"Type: {image_type}"
                cv2.putText(display_image, type_text, (10, 60), 
...
def main():
    ...
    annotator = BCCFaceMaskAnnotator()
